...
pre_screen_names <- function(name_matches, address_matches, lease_count,
    ...
    mutate(sum_n = sum(n.x, n.y, na.rm = TRUE)) %>%
	...
	# but now known to be correct, and pairs inferred to be correct via cluster
	...
	if(file.exists(file.path(ddir,
		...
		previous_non_pairs <-
			df %>%
			mutate(importance_dist = as.numeric(importance_dist)) %>%
